...
NUM_LAYERS = 3
from utils import CharTokenizer, CustomTokenizer
...
def convert_examples_to_features(examples, tokenizer, args,stage=None):
    ...
    for example_index, example in enumerate(examples):
        ...
        if stage=="test":
            ...
            target_tokens = ctok.tokenize(example.target)[:args.max_target_length-2]    # customized tokenization
        # target_tokens = [tokenizer.cls_token]+target_tokens+[tokenizer.sep_token]
        target_tokens = [ctok.cls_token]+target_tokens+[ctok.sep_token]
        # target_ids = tokenizer.convert_tokens_to_ids(target_tokens)
        target_ids = ctok.convert_tokens_to_ids(target_tokens)
                ...
                for batch in eval_dataloader:
                    ...
                    with torch.no_grad():
                        ...
                        for pred in preds:
                            ...
                            if 0 in t:
                                t=t[:t.index(0)]
                            text = ctok.decode(t,clean_up_tokenization_spaces=False)
                            p.append(text)
                model.train()
                predictions=[]
                accs=[]
