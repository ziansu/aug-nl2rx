# multi-modal to regular expression
...
from utils import restore_gold
...
def convert_examples_to_features(examples, tokenizer, args, stage=None):
    ...
    for example_index, example in enumerate(examples):
        ...
        if stage=="test":
            ...
            target_tokens = tokenizer.tokenize("None") # FIXME: None might not be suitable
        else:
        ...
        try:          
            ...
            target_tokens = [tokenizer.cls_token]+target_tokens+[tokenizer.sep_token]
            target_ids = tokenizer.convert_tokens_to_ids(target_tokens)
            print(target_ids[target_tokens.index('10')])
            assert 0
...
def main():
    ...
    parser.add_argument("--output_beam", action='store_true',
                        help="Whether to output all sequence in the beam.")
            ...
            if args.do_eval and ((global_step + 1) %args.eval_steps == 0) and eval_flag:
                ...
                with open(os.path.join(args.output_dir,"dev.output"),'w') as f, open(os.path.join(args.output_dir,"dev.gold"),'w') as f1:
                    for ref,gold in zip(p,eval_examples):
                        # gold_target = restore_gold(gold.target)
                    ...
                    for pred in preds:
                        if not args.output_beam:
                            ...
                            for i in range(pred.shape[0]):
                                ...
                                beam_text.append(restore_gold(text))
                            p.append(' '.join(beam_text))
            model.train()
            predictions=[]
            accs=[]
